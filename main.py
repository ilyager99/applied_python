...
if upload_file is not None:
    ...
    df['roll_temp'] = df.groupby(['city', 'season'])['temperature'].transform(lambda x: x.rolling(window=30).mean())
df['roll_temp'] = df['roll_temp'].fillna(df['temperature'])
temp_data = df.groupby(['city', 'season'])['roll_temp'].agg(['mean', 'std']).reset_index()
...
if selected_city:
    ...
    fig = px.line(season_year_filtered_df, x='timestamp', y='temperature',
                  ...
                  title=f'Температурный ряд сезона',
                  hover_data=['season'],
